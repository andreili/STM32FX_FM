...
public:
    ...
    typedef struct
    {
      ...
      EPID      data_pid;      /*!< Initial data PID.
                                    This parameter must be a number between Min_Data = 0 and Max_Data = 1      */
    ...
    FORCE_INLINE void HC_set_params(uint8_t ch_num, uint8_t ep_num, uint8_t dev_address, EOTGSpeed speed, EEPType ep_type, uint16_t mps)
        {
            m_regs->channels[ch_num].HCCHAR = ((dev_address << USB_OTG_HCCHAR_DAD_Pos) & USB_OTG_HCCHAR_DAD) |
                    ...
                    ((static_cast<uint32_t>(ep_type) << USB_OTG_HCCHAR_EPTYP_Pos) & USB_OTG_HCCHAR_EPTYP) |
...
private:
    ...
    FORCE_INLINE void set_dev_speed(EOTGSpeed speed) { m_regs->device.DCFG |= static_cast<uint32_t>(speed); }
    ...
    void EP0_start_Xfer(bool ep_is_in, uint8_t ep_num, uint16_t ep_maxpacket,
    ...
    FORCE_INLINE EOTGDeviceMode get_mode() { return static_cast<EOTGDeviceMode>(m_regs->global.GINTSTS & 0x1); }
