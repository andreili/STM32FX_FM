...
public:
    ...
    enum class ECTRL: uint8_t
    {
        ...
        TRIM                = 4,	/* Inform device that the data on the block of sectors is no longer used (needed at _USE_TRIM == 1) */

        /* Generic command (Not used by FatFs) */
        POWER               = 5,	/* Get/Set power status */
        LOCK                = 6,	/* Lock/Unlock media removal */
        EJECT               = 7,	/* Eject media */
        FORMAT              = 8,	/* Create physical format on the media */
    ...
    #endif
    #if _USE_IOCTL == 1
    static DSTATUS disk_ioctl(BYTE pdrv, FAT_FS::ECTRL cmd, void *buff);
    #endif
